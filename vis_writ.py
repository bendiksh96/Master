...
class Vis:
    ...
    def extract_data(self):
        ...
        if len(self.likelihood) > self.nfe_max:
            self.likelihood = self.likelihood[0:int(self.nfe_max)]
            self.individual = self.individual[0:int(self.nfe_max),:]
            self.index      = self.index[0:int(self.nfe_max)]
    ...
    def visualize_parameter_space(self):
        ...
        self.individual     = self.individual[sort_,:].reshape((len(self.likelihood), self.dim))
    ...
    def bin_parameter_space(self, bin_path):
        ...
        if self.dim == 3:
            ...
            bin_space = np.linspace(self.x_min, self.x_max, self.nbin_per_dim)
            x = 0
        ...
        if self.dim == 5:
            self.bin_arr = np.zeros((self.nbin_per_dim,self.nbin_per_dim,self.nbin_per_dim, self.nbin_per_dim,self.nbin_per_dim))
            self.bin_val = np.ones((self.nbin_per_dim,self.nbin_per_dim,self.nbin_per_dim,self.nbin_per_dim, self.nbin_per_dim))*self.max_val
            bin_space = np.linspace(self.x_min, self.x_max, self.nbin_per_dim)
            for p in range(len(self.likelihood)):
                var = []
                for j in range(self.dim):
                ...
                self.bin_arr[var[0],var[1],var[2], var[3],var[4]] += 1
                if self.bin_val[var[0],var[1],var[2],var[3],var[4]] > self.likelihood[p]:
                    self.bin_val[var[0],var[1],var[2], var[3], var[4]] = self.likelihood[p]
    ...
    def compare_bins(self, bin_path):
        ...
        if self.dim == 3:
            self.validation_likelihood = np.reshape(self.validation_likelihood, (100,100,100))
        if self.dim == 4:
            self.validation_likelihood = np.reshape(self.validation_likelihood, (100,100,100,100))
        if self.dim == 5:
        ...
        if self.dim == 5:
            for i in range(self.nbin_per_dim):
                for j in range(self.nbin_per_dim):
                    for k in range(self.nbin_per_dim):
                        for h in range(self.nbin_per_dim):
                            for l in range(self.nbin_per_dim):
                                #Row[0] ~ Occupancy
                                #Row[1] ~ Likelihood
                                ...
                                if self.validation_likelihood[i,j,k,h,l] < self.likelihood_threshold:
                                    #Number of bins in validation within the threshold

                                    if self.bin_val[i,j,k,h,l] < self.max_val:
                                        self.occ += (self.bin_val[i,j,k,h,l] - self.validation_likelihood[i,j,k,h,l])
                                        #Number of bins of the method within the bins
                                        bin_count_sigm +=1
                                    bin_count_sigm_valid +=1
