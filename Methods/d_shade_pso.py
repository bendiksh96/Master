...
class d_SHADE_pso:
    ...
    def init_particle(self):
        ...
        for i in range(self.num_ind):            
            for j in range(self.dim):
                ...
                self.velocity[i,j] = (self.individual[i,j] - self.optimal_individual[i,j]) * ru * 1e-3
    ...
    def evolve_particle(self):
        ...
        for i in range(self.num_ind):
            k = 1e-3
            for j in range(self.dim):
                # print(-k * (self.likelihood[i] - self.optimum))# * (self.individual[i,j] - self.optimal_individual[i,j]))
                # exit()
